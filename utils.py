...
def read_examples(filename):
    ...
    for i in range(len(exp)):
        code = exp[i]["span_diff_code"]
        code = ''.join([str(x) for x in code])
        ast = exp[i]["ast_diff"]
        ast = ' '.join([str(x) for x in ast])
        pun = '\\n'
        old_com = exp[i]["old_comment"]
        old_com = (re.sub(r"[%s]+" % pun, " ", old_com))
        src_dec = old_com + ' <s> ' + code + ' <s> ' + ast
        nl = exp[i]['new_comment']
        nl = (re.sub(r"[%s]+" % pun, " ", nl))
        examples.append(
            Example(
                idx=i,
                source=src_dec,
